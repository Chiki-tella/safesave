...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const [groupSavings, setGroupSavings] = useState<number>(0);
  ...
  useEffect(() => {
    const loadGroup = () => {
      try {
        ...
        if (!Array.isArray(approved)) { setGroupSavings(0); return; }
        const total = approved.reduce((acc: number, m: any) => {
          if (Array.isArray(m.savings) && m.savings.length) return acc + m.savings.reduce((s: number, d: any) => s + Number(d.amount || 0), 0);
          return acc + Number(m.totalSavings || 0);
        }, 0);
        setGroupSavings(total);
      } catch {
        setGroupSavings(0);
      }
    };
    ...
    const onStorage = (e: StorageEvent) => {
      if (!e.key) return;
      if (e.key.startsWith('safe_save_approved_members') || e.key === 'safe_save_users') loadGroup();
    };
    const onCustom = () => loadGroup();
    ...
    return () => { window.removeEventListener('storage', onStorage); window.removeEventListener('safe_save_approved_members_updated', onCustom as EventListener); window.removeEventListener('safe_save_users_updated', onCustom as EventListener); };
  }, []);
      ...
      <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 px-6 pt-12 pb-8 rounded-b-[2rem]">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
            ...
            <h3 className="text-white">₳{groupSavings.toLocaleString()}</h3>
            <div className="flex items-center gap-1 mt-2">
              <ArrowUpRight className="w-4 h-4 text-emerald-200" />
