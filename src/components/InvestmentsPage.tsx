...
export function InvestmentsPage() {
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'safe_save_investments') handleCustom();
    };

    window.addEventListener('safe_save_investments_updated', handleCustom as EventListener);
    window.addEventListener('storage', onStorage);

    return () => {
      window.removeEventListener('safe_save_investments_updated', handleCustom as EventListener);
      ...
      <div className="px-6 mb-8">
        ...
        <div className="space-y-4">
          {investments?.map((investment) => {
            ...
            const InvestmentIcon = investment.icon || Building2;
                ...
                {investment.status !== 'pending' && (
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-slate-800 text-sm">{Number(investment.progress || 0)}%</span>
                    </div>
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className={`h-full ${investment.color || 'bg-amber-500'} rounded-full`}
                        style={{ width: `${Number(investment.progress || 0)}%` }}
                      />
                    </div>
                  </div>
                )}
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <div className="bg-purple-50 rounded-xl p-3">
                  ...
                  <p className="text-purple-800">{Number(selectedInvestment.expectedROI || 0)}%</p>
                </div>
                <div className="bg-amber-50 rounded-xl p-3">
                  <p className="text-amber-700 text-xs mb-1">Project Status</p>
                  <p className="text-amber-800">{Number(selectedInvestment.progress || 0)}% Complete</p>
                </div>
              </div>
            </div>
